            ...
            <div class="card-body">
                <div class="row">
                    <div class="col">
                        ...
                        <span class="h2 font-weight-bold mb-0">{{ currency }} {{ calculateRevenue | formatCurrency }}</span>
                    </div>
                    <div class="col-auto">
                ...
                <div class="progress-wrapper" v-for="(group, index) in groups">
                    <div class="progress-info">
                        ...
                        <div class="progress-percentage">
                            <span>{{ currency }} {{ calculateByIntegration[index] | formatCurrency }}</span>
                        </div>
                    </div>
                    <div class="progress">
        ...
        <div v-show="!custom" class="card">
            <div class="card-body">
                <div class="row">
                    <div class="col">
                        <h5 class="card-title text-uppercase text-muted mb-0">Revenue</h5>
                        <span class="h1 font-weight-bold mb-0">{{ currency }} {{ calculateRevenue | formatCurrency }}</span>
                    </div>
                </div>
...
<script>
    ...
    export default {
        ...
        extends: Line,
        props: ['labels', 'revenue', 'currency', 'custom', 'groups', 'type'],
        filters: {
        ...
        computed: {
            ...
            calculateByIntegrationPeriod: function ()
            {
                let arr = [];

                $.each(this.groups, function(key, data) {
                    $.each(data, function(index, value) {
                        arr[key] = value.total_revenue;
                        return false;
                    });
                });
        ...
        watch: {
            revenue()
            {
                this.data.datasets[0].data = this.revenue;
                this.$data._chart.update();

                let revenue = this.revenue;
                this.revenueDate = revenue[0];
        ...
        methods: {
            calculatePercentage: function()
            {
                ...
                let percentage = ((revenue[0] - revenue[1]) / revenue[1]) * 100;
