<template>
    <span>
        <template v-if="status === 'pending'">
            <b-button variant="success" class="mt-2" size="sm" @click="openShipModel()" :class="{ disabled: disableBulkShip() }"><i class="fas fa-check"></i> Ready to Ship</b-button>
        </template>

        <b-modal id="ship-order-modal" ref="ship-order-modal" size="lg"
                 header-bg-variant="primary" hide-backdrop no-close-on-backdrop no-close-on-esc no-enforce-focus>

            <template v-slot:modal-header="{ close }">
                <h2 class="mb-0 text-white">Ship Order</h2>
                <button type="button" class="close" @click="closeShip" aria-label="Close">
            ...
            <template v-slot:modal-footer="{ ok, cancel }">
                <b-button variant="danger" @click="closeShip">Cancel</b-button>
                <b-button variant="primary" class="ml-auto" @click="confirmShip">Ship</b-button>
            </template>
        </b-modal>
    </span>
</template>
...
<script>
    export default {
        name: "LazadaBulkFulfillmentOrderComponent",
        ...
        methods: {
            ...
            closeShip() {
                this.$refs['ship-order-modal'].hide();
            },
            openShipModel() {
                if (this.selected_orders[this.status] && Object.values(this.selected_orders[this.status]).length <= 0) {
                    notify('top', 'Error', 'You need to select at least one order to ship.', 'center', 'danger');
                    return;
                }
                let error = false;
                Object.values(this.selected_orders[this.status]).map((order) => {
                    order.items.every((item) => {
                        if (item.fulfillment_status !== 1) {
                            ...
                            notify('top', 'Error', 'Order ['+ order_id +'] does not support shipping', 'center', 'danger');
                            return false;
                        }
                    });
                });
                ...
                this.$refs['ship-order-modal'].show();
            },
            confirmShip() {
                ...
                Object.values(this.selected_orders[this.status]).forEach((order) => {
                    ...
                    promisedEvents.push(axios.post('/web/orders/' + order.id + '/lazada/rts', {
