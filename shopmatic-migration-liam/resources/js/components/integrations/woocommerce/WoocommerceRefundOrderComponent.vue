<template>
    <span>
        <b-button variant="info" class="mr-2" @click="refund"><i class="fas fa-redo"></i> Refund</b-button>
...
<script>
    export default {
        name: "WoocommerceRefundOrderComponent",
        props: ['order'],
        data() {
            return {
                amount: null,
        ...
        computed: {
            canRefund() {
                if (this.order.fulfillment_status > 0 && this.order.fulfillment_status < 11) {
                    return true;
                }
                return false;
            },
        ...
        methods: {
            ...
            refund() {
                let title = 'Are you sure to refund the order?';
                let text = 'Confirm to refund?';
                ...
                swal.fire({
                    ...
                    if (result.value) {
                        ...
                        axios.post('/web/orders/' + this.order.id + '/woocommerce/refund', {}).then((response) => {
                            let data = response.data;
                            if (data.meta.error) {
                                notify('top', 'Error', data.meta.message, 'center', 'danger');
                            } else {
                                notify('top', 'Success', 'Successfully refund order!', 'center', 'success');
            ...
            /*confirmRefund() {
                if (!this.amount) {
                    notify('top', 'Error', 'Please enter the refund amount.', 'center', 'danger');
                    return;
                }
                if (this.sending_request) {
                    return;
                ...
                this.sending_request = true;
                axios.post('/web/orders/' + this.order.id + '/woocommerce/refund', {
                    amount: this.amount,
                }).then((response) => {
                    let data = response.data;
                    if (data.meta.error) {
                        ...
                        notify('top', 'Success', 'Successfully refund order!', 'center', 'success');
                        this.closeCancel();
                        this.$parent.$parent.$options.methods.updateCurrent();
                    }
                    this.sending_request = false;
                }).catch((error) => {
                    if (error.response && error.response.data && error.response.data.meta) {
