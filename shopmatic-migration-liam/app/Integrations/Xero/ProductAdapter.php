...
class ProductAdapter extends AbstractProductAdapter
{
    ...
    public function import($importTask, $config)
    {
        ...
        do {
            ...
            if (!empty($importTask) && empty($importTask->total_products)) {
                $importTask->total_products = $response['pagination']['results'];
                $importTask->save();
            }
            if (empty($totalPages)) {
            ...
            foreach ($products as $product) {
                ...
                // Get the variants and append it to the product data
    ...
    public function sync()
    {
        ...
        do {
            ...
            if (empty($response['code'])) {
                ...
                set_log_extra('response', $response);
                throw new \Exception('Unable to retrieve products for Lazada');
            }
        } while (count($products) >= 50 && count($products) != 0);
    }
     ...
     *
     * @param \App\ProductListing $listing
     * @return bool
     * @throws \GuzzleHttp\Exception\GuzzleException
     * @throws \Exception
     */
